...
function renderDiscussionWord(){
  ...
  if(myRole==='SPECTATOR'){
    ...
    note.textContent='Host prati rundu i može otvoriti glasanje, ali nema riječ.';
    return;
  }
  ...
  if(discussionWordVisible){
    ...
    note.textContent=myRole==='IMPOSTOR' ? 'Ovo je tvoja impostor riječ — drži priču uvjerljivom.' : 'Ovo je tvoja riječ za ovu rundu.';
  } else {
    value.classList.add('hidden');
    value.textContent='TAPNI ZA PRIKAZ';
    btn.textContent='PRIKAŽI';
    note.textContent='Samo ti vidiš svoju riječ. Otvori je kad ti zatreba tijekom rasprave.';
  }
}
...
$('btn-disc-word-toggle').onclick=()=>{ discussionWordVisible=!discussionWordVisible; renderDiscussionWord(); };
