...
let voiceLevels={}, phaseTick=null, phaseEndsAt=0, phaseNow='';
...
function getOpenerName(){ const openerId=(G?.speakOrder||[])[0]; const opener=(G?.players||[]).find(p=>p.id===openerId); return opener?.name || 'Host bira tempo'; }
...
function clearPhaseTicker(){ if(phaseTick){ clearInterval(phaseTick); phaseTick=null; } phaseEndsAt=0; phaseNow=''; }
function syncPhaseTimer(phase, remaining){
  ...
  render();
  phaseTick = setInterval(render, 250);
}
...
function setVoiceLevel(pid, level){
  const safe=Math.max(0,Math.min(100,level||0));
...
$('btn-join').onclick=()=>{
  ...
  socket.emit('joinRoom',{code,playerName:name,playerToken:getSession().playerToken||'',roomPin:pin});
};
...
function renderLobby(){
  ...
  if(isHost){
    ...
    $('l-err').textContent=activePlayers.length<minPlayers?`Potrebno još ${minPlayers-activePlayers.length} igrač(a) jer host moderira rundu (min 4 ukupno).`:(!ok?'Čekanje da svi povezani igrači budu spremni...':'');
  } else {
  ...
  G.players.forEach(p=>{
    ...
    d.innerHTML=`<div class="av ${p.isHost?'r':''}">${ini(p.name)}</div><div class="pi"><div class="pn">${esc(p.name)}</div><div class="ps">${p.isHost?'Host moderator · ne igra ovu rundu':'Operative'}${offline?' · Offline':''}</div></div><div style="font-family:'Bebas Neue',sans-serif;font-size:15px;color:var(--tx3);margin-right:5px;">${sc}pt</div><div class="pactions">${action}${status}</div>`;
    list.appendChild(d);
  });
...
function requestPlayAgain(){
  openConfirm({
    ...
    confirmClass:'bp',
    onConfirm:()=>socket.emit('playAgain',{code:G.code})
  });
}
...
// ── SOCKET EVENTS ─────────────────────────────────────────────────────────────
socket.on('roomCreated',room=>{clearActionLoading();clearJoinErrors(); G=room;me=room.players[0];saveSession({code:room.code,name:me?.name,playerToken:room.playerToken});badge();show('s-lobby');renderLobby();renderCustomWordsPreview();updatePinPill();SFX.join();});
socket.on('roomJoined',room=>{clearActionLoading();clearJoinErrors(); G=room;me=meNow();saveSession({code:room.code,name:me?.name||$('j-name').value.trim(),playerToken:room.playerToken});badge();show('s-lobby');renderLobby();renderCustomWordsPreview();updatePinPill();SFX.join();});
socket.on('sessionResumed',room=>{clearActionLoading();clearJoinErrors(); G=room;me=meNow();myWord=room.myWord;myRole=room.myRole;saveSession({code:room.code,name:me?.name||$('j-name').value.trim(),playerToken:room.playerToken});badge();
...
socket.on('resumeFailed',()=>{clearActionLoading(); clearSession();});
